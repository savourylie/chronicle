...
- [x] Server-side data access functions (use service role client):
...
- [x] Client-side data access functions (use anon key client):
...
- [x] All functions handle Supabase errors and throw descriptive errors
...
## Acceptance Criteria
- [x] Types match the database schema from ticket 024
- [x] `createAnalysis` returns the inserted row with generated `id`
- [x] `updateAnalysisStatus` correctly transitions between statuses
- [x] `setAnalysisResult` stores the `CommitGroup` tree as JSONB
- [x] `getAnalysis` returns `null` for non-existent IDs (not an error)
- [x] Supabase errors are caught and re-thrown with context
