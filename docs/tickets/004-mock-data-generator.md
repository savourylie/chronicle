...
## Requirements
- [ ] Generate a root `CommitGroup` with 3 levels of nesting:
...
- [ ] Generate realistic `CommitNode` data:
...
- [ ] Populate all `CommitGroup.metadata` fields correctly:
...
- [ ] Export a `generateMockData()` function that returns a complete `CommitGroup` tree
- [ ] Export a static `MOCK_DATA` constant for deterministic use in tests

## Files to Create/Modify
- `src/lib/mock-data.ts` — generator function and static export

## Acceptance Criteria
- [ ] `generateMockData()` returns a valid `CommitGroup` root
- [ ] The tree has 3 levels of depth with realistic variety
- [ ] All metadata fields are correctly computed
- [ ] Total commit count is 80-150 nodes (enough to be visually interesting, not overwhelming)
- [ ] Type-checks pass with strict TypeScript
