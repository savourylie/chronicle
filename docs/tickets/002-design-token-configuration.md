...
## Requirements
- [ ] Define all color tokens as CSS custom properties in `globals.css`:
...
- [ ] Configure hard shadow utilities:
...
- [ ] Configure bounce timing function as a Tailwind utility:
  - `transition-bounce: cubic-bezier(0.34, 1.56, 0.64, 1)`
- [ ] Update shadcn/ui theme variables to reference the custom tokens
...
## Acceptance Criteria
- [ ] `bg-background` renders warm cream `#FFFDF5`
- [ ] `text-foreground` renders slate `#1E293B`
- [ ] `shadow-hard` applies a solid 4px offset shadow
- [ ] `font-heading` uses Outfit, `font-body` uses Plus Jakarta Sans
- [ ] `rounded-lg` maps to 24px border radius
- [ ] All DESIGN.md color tokens are accessible as Tailwind classes
