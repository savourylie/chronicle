...
## Requirements
- [x] Implement size encoding strategies:
...
- [x] Implement color encoding strategies:
...
- [x] Read encoding configuration from Zustand store (`encoding.size`, `encoding.color`)
- [x] Generate a color legend:
  - Show what each color represents in the current encoding mode
  - Position in the corner of the visualization area
- [x] Group circles: derive color from the dominant encoding of their children
...
## Acceptance Criteria
- [x] Switching size encoding visibly changes circle sizes
- [x] Switching color encoding visibly changes circle colors
- [x] Color legend accurately reflects the current encoding
- [x] Author colors are consistent (same author = same color across all views)
- [x] Recency gradient is visually clear (old vs. new)
- [x] Colors are accessible: not relying on color alone (shape/size adds meaning)
