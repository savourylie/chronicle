...
## Requirements
- [x] Clone a GitHub repo given an HTTPS URL:
...
## Acceptance Criteria
- [x] Public GitHub repos clone successfully into temp directories
- [x] Cleanup function removes the temp directory and all contents
- [x] Clone timeout is enforced (120s default)
- [x] Invalid repo URLs return a clear "repository not found" error
- [x] No shell injection is possible via repo URL or branch name
- [x] `child_process.execFile` is used (not `exec` or `execSync`)
