...
## Requirements
- [x] Create Supabase project configuration:
...
- [x] Verify connection works with a simple health-check query
...
- [x] Server client can insert and read from `analyses` table
- [x] Browser client can read from `analyses` table
- [x] RLS policies are in place (open read, service-role write)
- [x] Environment variables are documented in `.env.local.example`
