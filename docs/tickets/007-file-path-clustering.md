...
## Requirements
- [ ] Compute file-path similarity between commits:
...
- [ ] Cluster commits into groups based on directory overlap:
...
- [ ] Return `CommitGroup[]` where each group contains related commits

## Files to Create/Modify
- `src/lib/pipeline/file-clustering.ts` — clustering module

## Acceptance Criteria
- [ ] Commits touching `src/auth/login.ts` and `src/auth/session.ts` cluster together
- [ ] Commits in unrelated directories end up in separate clusters
- [ ] Large refactors (touching many dirs) are handled without creating one mega-cluster
- [ ] Cluster names are human-readable directory prefixes
- [ ] Processes 5k commits in under 2 seconds
